...
def main():
    ...
    dists = open("dist.out", "w")
    energy = open("energy.out", "w")
    col = open("col.out", "w")
    mom = open("momentum.out", "w")
    forc = open("forces.out", "w")
    ...
    for t in range(timesteps):
        ...
        # # rethermalize every 1000 steps
        # if t % 1000 == 0 and t != 0:
        #     rethermalize(500, meshes)
        ...
        # min_dist = checkDist(num_meshes, meshes)
        # dists.write(f"{min_dist}\n")
        
        # Call GKS-force code
        os.system("./gks-force.x")

        # Apply forces from gks-force.txt
        gks_forces = open("gks-force.txt", "r")
        pe = float(gks_forces.readline().strip())
        forces = np.zeros((num_meshes, 3))
        torques = np.zeros((num_meshes, 3))
        for i in range(num_meshes):
            force = gks_forces.readline().strip().split()
            for j in range(3):
                forces[i][j] = float(force[j]) * 10e10
        for i in range(num_meshes):
            torque = gks_forces.readline().strip().split()
            for j in range(3):
                torques[i][j] = float(torque[j]) * 10e10
        forc.write(f"{forces}  {torques}\n")
        for i in range(num_meshes):
            ...
            p.applyExternalTorque(meshes[i], -1, torques[i], p.WORLD_FRAME)


        ke, lin_mom, ang_mom = system_KE(meshes)
        te = ke + pe
        energy.write(f"{t}  {pe}  {ke}  {te}\n")
        mom.write(f"{t}  {lin_mom}  {ang_mom}  {lin_mom + ang_mom}\n")
...
def create_mesh(mesh_path, position, orientation = [0,0,0,1], mass=1589, scale=1):
    mesh = p.loadURDF("general.urdf", basePosition= position, baseOrientation=orientation, useFixedBase=0, globalScaling=scale)
...
def system_KE(meshes):
    ...
    for i in range(len(meshes)):
        ...
        lin_mom += np.array(lin_velocity) * mass
        ang_mom += np.array(ang_velocity) * inertia

        lin_total += np.linalg.norm(np.array(lin_velocity))
        ang_total += np.linalg.norm(np.array(ang_velocity))
    lin_mom = np.linalg.norm(lin_mom)
    ang_mom = np.linalg.norm(ang_mom)
    return tKE + rKE, lin_mom, ang_mom#[0]
...
def calculate_structure_factor(box_size, num_points, ray_size):
    ...
    for x in range(num_points):
       for y in range(num_points):
          for z in range(num_points):
             point = (x * lattice_size - box_size, y * lattice_size - box_size, z * lattice_size - box_size)
